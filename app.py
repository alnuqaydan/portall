...
# -*- coding: utf-8 -*-
"""
Hudhud KPI System - Simplified Application
Main entry point for the simplified web application
...
class HudhudKPIHandler(http.server.SimpleHTTPRequestHandler):
    """Custom HTTP handler for the Hudhud KPI System"""
    ...
    def do_GET(self):
        ...
        if self.path == '/' or self.path == '/index.html':
            ...
            self.end_headers()
            self.wfile.write(self.get_styles().encode())
        else:
            self.send_error(404)
...
def main():
    ...
    with socketserver.TCPServer(("", port), HudhudKPIHandler) as httpd:
        try:
            httpd.serve_forever()
